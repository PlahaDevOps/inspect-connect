...
import { CalendarDays, ChevronDown as ChevronDownLucide, ChevronRight as ChevronRightLucide, DollarSign, Folder, HelpCircle, LogOut, Mail, Settings, BarChart3, Rocket, ClipboardList } from 'lucide-react';
    ...
    const navSections: { subheader: string; items: NavItem[] }[] = [
        {
            subheader: 'DASHBOARD',
            items: [
                { label: 'Overview', to: '/inspector/dashboard', icon: <BarChart3 size={20} /> },
                { label: 'My Jobs', to: '/inspector/jobs', icon: <ClipboardList size={20} /> },
                { label: 'Calendar', to: '/inspector/calendar', icon: <CalendarDays size={20} /> },
                { label: 'Certificates', to: '/inspector/certificates', icon: <Folder size={20} /> },
                { label: 'Requests', to: '/inspector/requests', icon: <Rocket size={20} /> },
                { label: 'Payments', to: '/inspector/payments', icon: <DollarSign size={20} /> },
            ],
        },
        {
            subheader: 'HELP',
            items: [
                { label: 'Support', to: '/inspector/support', icon: <HelpCircle size={20} /> },
                { label: 'Contact Us', to: '/inspector/contact', icon: <Mail size={20} /> },
            ],
        },
    ];
    ...
    return (
        <Box sx={{ display: 'flex' }}>
            ...
            >
                        ...
                        <Tooltip title="Notifications">
                            <IconButton color="inherit" aria-label="notifications">
                                <Badge color="error" variant="dot" overlap="circular">
                                    {/* Reuse same bell icon from admin to keep consistency */}
                ...
                >
                    <MenuItem onClick={() => { handleCloseSettingsMenu(); navigate('/inspector/settings'); }}>General</MenuItem>
                    <MenuItem onClick={() => { handleCloseSettingsMenu(); navigate('/inspector/settings?tab=appearance'); }}>Appearance</MenuItem>
                    <MenuItem onClick={() => { handleCloseSettingsMenu(); navigate('/inspector/payments'); }}>Billing</MenuItem>
                </Menu>
                ...
                <Menu
                    ...
                    transformOrigin={{ vertical: 'top', horizontal: 'right' }}
                >
                    <MenuItem onClick={() => { handleCloseProfileMenu(); navigate('/inspector/profile'); }}>My Profile</MenuItem>
                    <MenuItem onClick={() => { handleCloseProfileMenu(); navigate('/inspector/account'); }}>Account Settings</MenuItem>
                    <MenuItem onClick={() => { handleCloseProfileMenu(); dispatch(logoutUser()); }}>Logout</MenuItem>
                </Menu>
            </AppBar>
            <Drawer variant="permanent" open={open}>
                <DrawerHeader>
                    <Stack onClick={() => navigate('/inspector/dashboard')} direction="row" alignItems="center" spacing={1.5} sx={{ flexGrow: 1, pl: 1, cursor: 'pointer' }}>
