...
export const signUpValidation = (data: IUser) => {
  const schema = Joi.object({
    ...
    certificateAgencyIds: Joi.alternatives().conditional('role', {
      is: 2,
      then: Joi.array().items(Joi.string().regex(/^[0-9a-fA-F]{24}$/)).min(1).required().messages({
    ...
    agreedToTerms: Joi.alternatives().conditional('role', {
      is: 1 || 2,
      then: Joi.boolean()
      .valid(true)
      .required()
    ...
    isTruthfully: Joi.alternatives().conditional('role', {
      is: 1 || 2,
      then: Joi.boolean()
      .valid(true)
      .required()
      .messages({
      'any.only': 'All the information provided must be true and correct',
