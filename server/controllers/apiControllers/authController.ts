...
import * as authServices from '../../services/apiServices/authService';
import * as authValidations from '../../validations/apiValidations/authValidations';
...
export const logout = async (req: any, res: Response): Promise<Response | void> => {
  ...
  try {
    if (!req.user) {
      return helper.failed(res, MESSAGES.AUTH.USER_NOT_FOUND);
    }
