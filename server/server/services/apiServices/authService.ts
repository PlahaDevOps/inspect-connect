...
import { resetEmailTemplate, welcomeEmail } from '../../utils/templates/emailTemplates';
...
import { createCustomer } from '../../utils/stripe/stripe';

export const signUpService = async (data: IUser) => {
  try {
    if(data.role === 0) {
      const adminExists = await userModel.findOne({ role: 0, isDeleted: false });
      if (adminExists) {
        return { error: MESSAGES.AUTH.ADMIN_ALREADY_EXISTS };
      }
    }
  ...
  const logoImageLink = `${process.env.CLIENT_BASE_URL}/images/mail/welcome-image.png`;
  const userName = user.name || 'Dear User';

  const mailData: IMailData = {
    to: data.email,
    subject: 'Inspect Connect - Welcome to our platform',
    text: 'Please click on the link below to verify your account',
    html: welcomeEmail(logoImageLink, userName),
  };
  ...
  let customerId = null;
  if(data.role == 2){
    customerId = await createCustomer(data.email);
  }
...
export const verifyOtpService = async (data: verifyOtpInput) => {
  try {
    ...
    return { message: MESSAGES.AUTH.VERIFY_OTP_SUCCESS };
...
export const resendOtpService = async (data: any) => {
  try {
    ...
    if(!result){
      return { error: MESSAGES.AUTH.TWILIO_ERROR };
    }
...
export const forgotPasswordService = async (data: any) => {
  try {
    ...
    const logoImageLink = `${process.env.CLIENT_BASE_URL}/images/mail/logo.png`;
    const resetPasswordLink = `${process.env.CLIENT_BASE_URL ? process.env.CLIENT_BASE_URL : 'http://localhost:5173'}/reset-password?email=${encryptedEmail}&token=${newToken}`;
    const userName = user.name || 'Dear User';
